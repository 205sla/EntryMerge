...
export async function POST(request: NextRequest) {
  try {
    ...
    // ─────────────────────────────────────────────────────────
    // 최종 메타데이터 업데이트
    const fileNames = projectJsons.map(p => p.name || '').filter(Boolean);
    
    // 최종 project.json 메타데이터 업데이트
    mergedProject.name = fileNames.join('-');
    mergedProject.updatedAt = new Date().toISOString();
    
    // 버전 처리: 가장 높은 버전 사용
    const versions = projectJsons
      .map(p => p.version || '1.0.0')
      .filter(Boolean);
    mergedProject.version = versions.reduce((max, current) => {
